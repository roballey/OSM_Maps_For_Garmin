...
# and extras).  Uses style files and type rules (from txt file) in the
# generation
#
##############################################################################
...
extra_dir="work/osmsplitmaps/extra"
...
echo "Converting OSM (+ extras if available) to Garmin for region ${region} using style ${style} and type ${type} rendering rules"
...
# Convert split OSM files and extras to split Garmin image files
#    - Assumes split OSM pbf files have been put in the directory osm_dir (build_split.sh does this)
echo "Converting split OSM files and extras into Garmin Image files ..."
